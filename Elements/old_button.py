...
class DefaultButton(Element):
    def __init__(self,
                 ...
                 text: str = None,
                 text_size: int = None,
                 font: str | pygame.font.Font = None,
    ...
    def update(self, dt: float) -> None:
        ...
        if self._state == c.States.DISABLED:
            ...
            if mouse_keys[0]:
                if self._state == c.States.HOVERED or self._prev_pressed:
        ...
        if self._state == c.States.PRESSED and \
                not self._prev_pressed and \
                self._on_click_function is not None:
    ...
    def set_color(self, key: str, color: Any) -> None:
        ...
        :param key: The requested color. Must be one of: ['disabled', 'disabled_text', 'pressed',
                    'hovered', 'hovered_outline', 'base']. It is not case sensitive.
        :param color: The color to set.
        """

        # check if key is correct
        colors = ['disabled', 'disabled_text', 'pressed',
                  'hovered', 'hovered_outline', 'base']
    ...
    def center(self, value: tuple[int, int]) -> None:
        ...
        :raises ValueError: If the center is invalid.
        """
        if not check_tools.is_pos(value):
            raise ValueError(f"center must be tuple[int, int] not '{value}'.")

        self._rect.center = value
        self._pos_x, self._pos_y = self._rect.topleft
